...
| 18 | `fissible/transmark-pdf` (separate package): `PdfReader` — best-effort layout-heuristic PDF → canonical tree (font-size/whitespace heuristics), throws on ambiguous extraction; library decision: `smalot/pdfparser` (LGPLv3) | L-XL | `PdfWriter` package scaffold (done) | [transmark-pdf#4](https://github.com/fissible/transmark-pdf/issues/4) | Planned — see `transmark-pdf` repo's `docs/superpowers/plans/2026-08-08-pdf-reader.md` |
